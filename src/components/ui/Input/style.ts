import { ColorScale } from "assets/style/types";
...
const StyledInput = styled.input<{ $placeholderScale: keyof ColorScale }>`
  font-size: inherit;
  ...
  ::placeholder {
    color: ${({ theme, disabled, $placeholderScale }) => (
    disabled
      ? theme.palette.action.disabled
      : `${theme.palette.grey[$placeholderScale]}75` // % of opacity
  )};
  }
`;
